...
RSpec.describe Task do
  ...
  describe "order" do
    ...
    let!(:first) { project.tasks.create!(project_order: 1, user_id: user.id) }
    let!(:second) { project.tasks.create!(project_order: 2, user_id: user.id) }
    let!(:third) { project.tasks.create!(project_order: 3, user_id: user.id) }
